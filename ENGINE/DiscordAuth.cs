    ...
    public static class DiscordAuth
    {
        ...
        public static async Task<LicenseStatus> ValidateLicenseAsync(string token)
        {
            try
            {
                ...
                req.Headers.Authorization = new System.Net.Http.Headers.AuthenticationHeaderValue("Bearer", token);
                var res  = await _http.SendAsync(req);
                if (!res.IsSuccessStatusCode) return LicenseStatus.FromLegacy(false, 0);
...
<div style='text-align:center;padding:40px;'>
  ...
  <script>
    ...
    public class LicenseStatus
    {
        ...
        // Backwards-compat: used for offline/error fallback states
        public static LicenseStatus FromLegacy(bool licensed, int trialDaysLeft = 0) =>
            new LicenseStatus(licensed, licensed, null, null, null, null,
                              trialDaysLeft > 0 ? (long?)trialDaysLeft : null);

        /// <summary>Short status string for display in the ENGINE UI.</summary>
        public string StatusLine()
        {
            ...
            if (!string.IsNullOrEmpty(DurationLabel)) return $"ACTIVE — {DurationLabel}";
            return "ACTIVE";
        }
    }
