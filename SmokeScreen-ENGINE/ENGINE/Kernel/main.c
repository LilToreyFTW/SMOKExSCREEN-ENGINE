...
// Custom IOCTL handlers for usermode communication
NTSTATUS HandleCustomIOCTL(PDEVICE_OBJECT DeviceObject, PIRP Irp) {
	...
	switch (ioControlCode) {
		case 0x800: // IOCTL_SPOOF_DISK
		case 0x801: // IOCTL_SPOOF_NETWORK
		case 0x802: // IOCTL_SPOOF_GPU
		case 0x803: // IOCTL_SPOOF_BIOS
		case 0x804: // IOCTL_SPOOF_BASEBOARD
		{
			PHWID_SPOOF_REQUEST request = (PHWID_SPOOF_REQUEST)Irp->AssociatedIrp.SystemBuffer;
			if (request && request->Length > 0) {
				// Apply the spoofed serial
				strcpy(SERIAL, request->NewSerial);
				printf("Applied spoofed HWID: %s -> %s\n", request->HardwareType, request->NewSerial);
			}
			break;
		}
		case 0x805: // IOCTL_UNSPOOF_ALL
		{
			// Restore original serials
			strcpy(SERIAL, "---------");
			printf("Unspoofed all HWIDs\n");
			break;
		}
		default:
...
NTSTATUS DriverEntry(PDRIVER_OBJECT driver, PUNICODE_STRING registry_path) {
	...
	// Create device for usermode communication
	UNICODE_STRING deviceName = RTL_CONSTANT_STRING(L"\\Device\\Kernel");
	...
	if (NT_SUCCESS(status)) {
		UNICODE_STRING symLinkName = RTL_CONSTANT_STRING(L"\\??\\Kernel");
		status = IoCreateSymbolicLink(&symLinkName, &deviceName);
		if (NT_SUCCESS(status)) {
			driver->MajorFunction[IRP_MJ_DEVICE_CONTROL] = HandleCustomIOCTL;
			printf("Device created for usermode communication\n");
		} else {
			printf("Failed to create symbolic link: %p\n", status);
		}
	} else {
		printf("Failed to create device: %p\n", status);
	}
