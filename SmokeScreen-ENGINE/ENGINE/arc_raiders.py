...
class ArcRaidersController:
    def __init__(self):
        self.name = "ArcRaiders"
    ...
    def _recoil_loop(self):
        while self.script_running and self.rbutton_held and self.lbutton_held:
            ...
            progress = min(self.shot_count / 55, 1.0)
...
def main():
    ...
    controller = ArcRaidersController()
    listener = Listener(on_click=controller.on_click)
    listener.start()

    print(f"[Arc Raiders] Controller started. Press ESC to exit.")
    print(
        f"[Arc Raiders] Hold LEFT + RIGHT mouse button to activate recoil compensation."
    )
    ...
    try:
        ...
        controller.stop()
        listener.stop()
        print("[Arc Raiders] Controller stopped.")
