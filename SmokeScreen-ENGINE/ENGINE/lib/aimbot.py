...
# If you use stretched res, hardcode X and Y. For example: screen_res_x = 1234
...
use_trigger_bot = True # Will shoot if crosshair is locked on to the player
...
class Aimbot:
    ...
    def __init__(self, box_constant = fov, collect_data = False, mouse_delay = 0.0009):
        ...
        self.box_constant = box_constant #controls the size of the detection box (equaling width and height)
    ...
    def start(self):
        ...
        detection_box = {'left': int(half_screen_width - self.box_constant//2), #x1 coord (for top-left corner of box)
                          'top': int(half_screen_height - self.box_constant//2), #y1 coord (for top-left corner of box)
        ...
        while True:
            ...
            if len(result.boxes.xyxy) != 0: #player detected
                ...
                for box in result.boxes.xyxy: #iterate over each player detected
                    ...
                    x2y2 = (x2, y2)
                    height = y2 - y1
                    relative_head_X, relative_head_Y = int((x1 + x2)/2), int((y1 + y2)/2 - height/aim_height) # offset to roughly approximate head using a ratio of the height
...
if __name__ == "__main__": print("You are in the wrong directory and are running the wrong file; you must run fortnite_main.py")
