...
class FortniteController:
    ...
    def _recoil_loop(self):
        while self.script_running and self.rbutton_held and self.lbutton_held:
            ...
            progress = min(self.shot_count / 70, 1.0)
...
def main():
    ...
    controller = FortniteController()
    listener = Listener(on_click=controller.on_click)
    listener.start()

    print(f"[Fortnite] Controller started. Press ESC to exit.")
    print(f"[Fortnite] Hold LEFT + RIGHT mouse button to activate recoil compensation.")
    ...
    try:
        ...
        controller.stop()
        listener.stop()
        print("[Fortnite] Controller stopped.")
