...
class R6SController:
    ...
    def _recoil_loop(self):
        while self.script_running and self.rbutton_held and self.lbutton_held:
            ...
            progress = min(self.shot_count / 50, 1.0)
...
def main():
    ...
    controller = R6SController()
    listener = Listener(on_click=controller.on_click)
    listener.start()

    print(f"[R6S] Controller started. Press ESC to exit.")
    print(f"[R6S] Hold LEFT + RIGHT mouse button to activate recoil compensation.")
